...
# Define the network security group (NSG) with SSH and HTTP rules
...
# Define cloud-init script for Apache installation (Fix: Base64 encoding added)
data "cloudinit_config" "init" {
  gzip          = false
  base64_encode = true # Fix: Enable Base64 encoding
...
# Define the virtual machine (VM) and apply cloud-init
resource "azurerm_linux_virtual_machine" "vm" {
  ...
  # Fix: Ensure `custom_data` is Base64 encoded
